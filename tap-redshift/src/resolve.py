...
def desired_columns(selected, table_schema):
    ...
    if selected_but_unsupported:
        LOGGER.warning(
            f"Columns {selected_but_unsupported} were selected but are not supported. " +
            "Skipping them"
        )
    ...
    if selected_but_nonexistent:
        LOGGER.warning(
            f"Columns {selected_but_nonexistent} were selected but do not exist"
        )
...
def entry_is_selected(catalog_entry):
    ...
    return bool(
        catalog_entry.is_selected() or metadata.get(mdata, (), 'selected')
    )


# function to store property values in catalog for selected streams
...
def resolve_catalog(discovered, catalog, state):
    ...
    for catalog_entry in streams:
        ...
        if not discovered_table:
            LOGGER.warning(
                f"Database {catalog_entry.database} table {catalog_entry.table} " +
